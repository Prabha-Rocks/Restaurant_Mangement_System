...
# I have created this file - S.Prabha 
...
def index(request):
    ...
    prabha = {'allProds': allProds}
    return render(request, 'shop/index.html', prabha)
...
def search(request):
    ...
    for cat in cats:
        ...
        if len(prod) != 0:
            allProds.append([prod, range(1, nSlides), nSlides])
    prabha = {'allProds': allProds, "msg": ""}
    if len(allProds) == 0 or len(query) < 3:
        prabha = {'msg': "No item available. Please make sure to enter relevant search query"}
    return render(request, 'shop/search.html', prabha)
